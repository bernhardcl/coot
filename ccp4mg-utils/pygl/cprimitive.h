...
class Primitive{
 ...
 public:
  ...
  void set_line_scaling(double line_scaling_in){ line_scaling = line_scaling_in;};
  ...
  virtual const GLfloat* GetBufferVertices(void) const {return bufferVertices;};
  virtual const GLfloat* GetBufferNormals(void) const {return bufferNormals;};
  virtual const GLfloat* GetBufferColours(void) const {return bufferColors;};
  const GLfloat* GetBufferOrigins(void) const {return bufferOrigins;};
  const GLfloat* GetBufferSizes(void) const {return bufferSizes;};
  virtual const GLuint* GetBufferIndices(void) const {return bufferIndices;};
  virtual int GetNumberOfBufferVertices() const;
  virtual int GetNumberOfBufferIndices() const;
  virtual std::string GetPrimitiveTypeAsString(void) const;
...
class PointSphereCollection : public Primitive {
 protected:
  std::vector<Primitive*> points;
 public:
  PointSphereCollection();
  ~PointSphereCollection();
  void add_primitive(Primitive *point);
  void set_draw_colour(const GLfloat *col=0);
  void draw(const double *override_colour=0, int selective_override=0);
  bool isLine() const {return true;};
  bool isDots() const {return true;};
  std::vector<Primitive*> GetSimplePrimitives(const Volume &clip_vol, const matrix &objrotmatrix, const Cartesian &objorigin, int start=-1, int end=-1) const;
  std::vector<Primitive*> GetPrimitives() const {return points;}
  void SetPrimitives(std::vector<Primitive*> &points_in){points=points_in;}
  size_t GetNumberOfSimplePrimitives() const { return points.size();} ;
...
class CylinderCollection : public PolyCollection {
 ...
 public:
  ...
  virtual std::vector<Primitive*> GetSimplePrimitivesArrays(const Volume &clip_vol, const matrix &objrotmatrix, const Cartesian &objorigin, int start=-1, int end=-1) ;
   ...
   void SetCapped(int capped_in) {capped=capped_in;}; 
   bool GetCapped() const { return capped;}; 
};
