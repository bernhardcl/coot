...
void Displayobject::draw_text_background(const Quat &quat_in, double radius, double ox, double oy, double oz, double fontScaling){
  ...
  while(k!=text_prims.end()){
    if(!(*k)->isMultiColoured()){
      ...
      if( (*k)->GetTextureID()==0||(*k)->GetTextureID_B()==0) (*k)->initialize();
      ...
      glBegin(GL_QUADS);
      glVertex3f(0,size_offset,1);
      glVertex3f(2*size*ratio,size_offset,1);
      glVertex3f(2*size*ratio,2*size+size_offset,1);
      glVertex3f(0,2*size+size_offset,1);
      glEnd();
      glPopMatrix();
    }
#ifdef __APPLE_CC__
  ...
  while(k!=text_prims.end()){
	...
	size_offset = 0;
      ...
      glBegin(GL_QUADS);
      glVertex3f(x_cent_off,y_cent_off+size_offset,1);
      glVertex3f(x_cent_off+2*size*ratio,y_cent_off+size_offset,1);
      glVertex3f(x_cent_off+2*size*ratio,y_cent_off+2*size+size_offset,1);
      glVertex3f(x_cent_off,y_cent_off+2*size+size_offset,1);
      glEnd();
      glPopMatrix();
    }
    k++;
...
void Displayobject::draw_text(const Quat &quat_in, double radius, double ox, double oy, double oz, double fontScaling,int nCustomPlanes){
  ...
  while(k!=text_prims.end()){
    if(!(*k)->isMultiColoured()){
      ...
      glBegin(GL_QUADS);
      glTexCoord2f(0,0);
      glVertex3f(0,size_offset,2);
      glTexCoord2f(1,0);
      glVertex3f(2*size*ratio,size_offset,2);
      glTexCoord2f(1,1);
      glVertex3f(2*size*ratio,2*size+size_offset,2);
      glTexCoord2f(0,1);
      glVertex3f(0,2*size+size_offset,2);
      //(*k)->draw();
      glEnd();
      glPopMatrix();
    }
  ...
  while(k!=text_prims.end()){
    if((*k)->isMultiColoured()){
      ...
      if((*k)->IsBillBoard()){
        glLoadIdentity();
        GLdouble x = (v.get_x()-.5)*(r-l);
        GLdouble y_coord = (v.get_y()-.5)*(b-t);
        GLdouble z =  -n-3;
	size_offset = 0;
        size = fabs(win_h*pix_h/view_h)*fontScaling;
        glTranslatef(x,y_coord,z);
      } else {
        glTranslatef(v.get_x(),v.get_y(),v.get_z());
        glMultMatrixd(matinv);
      }
      ...
      glBegin(GL_QUADS);
      glTexCoord2f(0,0);
      glVertex3f(x_cent_off,y_cent_off+size_offset,2);
      glTexCoord2f(1,0);
      glVertex3f(x_cent_off+2*size*ratio,y_cent_off+size_offset,2);
      glTexCoord2f(1,1);
      glVertex3f(x_cent_off+2*size*ratio,y_cent_off+2*size+size_offset,2);
      glTexCoord2f(0,1);
      glVertex3f(x_cent_off,y_cent_off+2*size+size_offset,2);
      //(*k)->draw();
      glEnd();
      glPopMatrix();
    }
...
void Displayobject::draw_lines(double *override_colour, int transparent_in, int selective_override, double line_scaling) const {
  ...
  while(k!=prims.end()){
    if((*k)->isLine()){
      ...
      (*k)->set_line_scaling(line_scaling);
