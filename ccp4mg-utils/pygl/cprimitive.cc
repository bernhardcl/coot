...
void LineCollection::SetAlpha(double alpha_in){
  //std::cout << "LineCollection::SetAlpha " << alpha_in << "\n";
  alpha = alpha_in;
  std::vector<Primitive*>::iterator k = lines.begin();
  while(k!=lines.end()){
    (*k)->SetAlpha(alpha);
    k++;
  }
...
CylinderCollection::CylinderCollection() : PolyCollection(){cylinder_accu=1;capped=false;}
CylinderCollection::~CylinderCollection(){ }
DashCylinderCollection::DashCylinderCollection() : CylinderCollection(){cylinder_accu=1;capped=false;}
...
void get_elliptical_ribbon(std::vector<Primitive*> &a,const std::vector<Cartesian> &vertices_tmp,const std::vector<std::vector<Cartesian> > &AB,int my_accu,int textured,int multicolour,const std::vector<Cartesian> &colour_vector_tmp, const double *colp, double alpha, const bool two_colour){
  double col[4];
  int idx = vertices_tmp.size()/2;
  Cartesian p_t = vertices_tmp[idx+1] + vertices_tmp[idx-1] - 2*vertices_tmp[idx];
  p_t.normalize();
  ...
  for(int i=0;i<360;i=i+360/my_accu){
    ...
    std::vector<Cartesian> v;
    std::vector<Cartesian> ns;
    if(two_colour&&((i<180&&face_one)||(i>180&&!face_one))){
      colp = colour;
    }
    for(unsigned int j=0;j<vertices_tmp.size();j++){
      if(multicolour&&(two_colour&&((i>180&&face_one)||(i<180&&!face_one)))||!two_colour){
        col[0] = colour_vector_tmp[j].get_x();
        col[1] = colour_vector_tmp[j].get_y();
        col[2] = colour_vector_tmp[j].get_z();
        col[3] = colour_vector_tmp[j].get_a();
...
std::vector<Primitive*> DashCylinderCollection::GetSimplePrimitivesArrays(const Volume &clip_vol, const matrix &objrotmatrix, const Cartesian &objorigin, int start, int end) {
  ...
  // This is arguably not-optimal, but it is the simplest way of doing this.
  unsigned nCylinders = prims.size();
  std::vector<Cartesian> segcarts(2);
  CylinderCollection *l = new CylinderCollection();
  for(unsigned k=0;k<nCylinders;k++){
    std::vector<Cartesian> carts = prims[k]->GetVertices();
    Cartesian p = carts[1] - carts[0];
    double length = p.length();
    int nsegments = (int)(length/dash_length);
    if((nsegments%2) == 1) nsegments++;
    //std::cout << "Split a cylinder here into " << nsegments << std::endl;
    for(int j=0;j<nsegments;j++){
      if(j%2 >0){
        double frac = (j+0.5)/(double)nsegments;
        double frac2 =(j+1.5)/(double)nsegments;
        segcarts[0] = frac * carts[0] + (1.0-frac)*carts[1];
        segcarts[1] = frac2 * carts[0] + (1.0-frac2)*carts[1];
        CylinderElement *line = new CylinderElement(segcarts,prims[k]->GetColour(),segcarts[0],prims[k]->GetSize(),prims[k]->GetAlpha(),cylinder_accu);
        l->add_primitive(line);
      }
    }
  }
  l->SetAccu(cylinder_accu);
  l->SetCapped(capped);
  std::vector<Primitive*> a = l->GetSimplePrimitivesArrays(clip_vol, objrotmatrix, objorigin, start, end);
  
  nRealVerts = l->GetNumberOfBufferVertices();
  nVerts = l->GetNumberOfBufferIndices();
  ...
  memcpy(bufferVertices,l->GetBufferVertices(),nRealVerts*sizeof(GLfloat));
  memcpy(bufferNormals,l->GetBufferNormals(),nRealVerts*sizeof(GLfloat));
  memcpy(bufferColors,l->GetBufferColours(),4*nRealVerts/3*sizeof(GLfloat));
  memcpy(bufferIndices,l->GetBufferIndices(),nVerts*sizeof(GLfloat));
...
std::vector<Primitive*> CylinderCollection::GetSimplePrimitivesArrays(const Volume &clip_vol, const matrix &objrotmatrix, const Cartesian &objorigin, int start, int end) {
  ...
  if(capped){
    bufferVertices = new float[3* 6*my_accu*nCylinders];
    bufferNormals = new float[3* 6*my_accu*nCylinders];
    bufferColors = new float[3* 8*my_accu*nCylinders];
    bufferIndices = new GLuint[3* (2*(my_accu+1)*nCylinders + 2*nCylinders)];
  } else {
  ...
  if(capped){
  int indexOffsetTot = 2*nCylinders*(my_accu);
  ...
  for(unsigned k=0;k<nCylinders;k++){
    ...
    up.normalize(prims[k]->GetSize());
    right.normalize(prims[k]->GetSize());
    int indexOffset = 2*k*(my_accu) + indexOffsetTot;
    for(int j=0;j<=360;j=j+360/my_accu,jj++){
      if(jj==my_accu){
        bufferIndices[indexOffsetInternal++] = indexOffset + 1;
        bufferIndices[indexOffsetInternal++] = indexOffset;
      ...
      if(jj<my_accu){
        ...
        double x = cos(theta);
        double y = sin(theta);
        Cartesian p2 = x*up + y*right + carts[0];
        Cartesian p1 = carts[0];
        Cartesian n1 = -p;
        bufferVertices[nRealVerts+6*k*my_accu+6*jj]   = p1.get_x();
        bufferVertices[nRealVerts+6*k*my_accu+6*jj+1] = p1.get_y();
        bufferVertices[nRealVerts+6*k*my_accu+6*jj+2] = p1.get_z();
        bufferVertices[nRealVerts+6*k*my_accu+6*jj+3] = p2.get_x();
        bufferVertices[nRealVerts+6*k*my_accu+6*jj+4] = p2.get_y();
        bufferVertices[nRealVerts+6*k*my_accu+6*jj+5] = p2.get_z();
        bufferNormals[nRealVerts+6*k*my_accu+6*jj]   = n1.get_x();
        bufferNormals[nRealVerts+6*k*my_accu+6*jj+1] = n1.get_y();
        bufferNormals[nRealVerts+6*k*my_accu+6*jj+2] = n1.get_z();
        bufferNormals[nRealVerts+6*k*my_accu+6*jj+3] = n1.get_x();
        bufferNormals[nRealVerts+6*k*my_accu+6*jj+4] = n1.get_y();
        bufferNormals[nRealVerts+6*k*my_accu+6*jj+5] = n1.get_z();
        maxInd = nRealVerts+6*k*my_accu+6*jj+6;
        bufferColors[nRealVerts*4/3+8*k*my_accu+8*jj  ] = prims[k]->GetColour()[0];
        bufferColors[nRealVerts*4/3+8*k*my_accu+8*jj+1] = prims[k]->GetColour()[1];
        bufferColors[nRealVerts*4/3+8*k*my_accu+8*jj+2] = prims[k]->GetColour()[2];
        bufferColors[nRealVerts*4/3+8*k*my_accu+8*jj+3] = alpha;
        bufferColors[nRealVerts*4/3+8*k*my_accu+8*jj+4] = prims[k]->GetColour()[0];
        bufferColors[nRealVerts*4/3+8*k*my_accu+8*jj+5] = prims[k]->GetColour()[1];
        bufferColors[nRealVerts*4/3+8*k*my_accu+8*jj+6] = prims[k]->GetColour()[2];
        bufferColors[nRealVerts*4/3+8*k*my_accu+8*jj+7] = alpha;
      }
    }
  }

  indexOffsetTot += 2*nCylinders*(my_accu);
...
PointSphereCollection::PointSphereCollection() : Primitive(){}
PointSphereCollection::~PointSphereCollection(){
  std::vector<Primitive*>::iterator prim_iter = points.begin();
  while(prim_iter!=points.end()){
    delete *prim_iter;
    prim_iter++;
  }
  points.clear();
}
void PointSphereCollection::set_draw_colour(const GLfloat *col){
}
void PointSphereCollection::add_primitive(Primitive *point){
  points.push_back(point);
}
void PointSphereCollection::draw(const double *override_colour, int selective_override){
}
std::vector<Primitive*> PointSphereCollection::GetSimplePrimitives(const Volume &clip_vol, const matrix &objrotmatrix, const Cartesian &objorigin, int start, int end) const {
  //std::cout << "PointSphereCollection::GetSimplePrimitives\n";
  std::vector<Primitive*> a;
  if(points.size()<1) return a;
  std::vector<Primitive*>::const_iterator prim_iter = points.begin();
  while(prim_iter!=points.end()){
    //(*prim_iter)->set_transparent(transparent);
    //(*prim_iter)->SetAlpha(alpha);
    std::vector<Primitive*> b = (*prim_iter)->GetSimplePrimitives(clip_vol,objrotmatrix,objorigin,start,end);
...
std::string PointSphereCollection::GetPrimitiveTypeAsString(void) const {
    return std::string("POINTS_SPHERES");
}
std::vector<Primitive*> PointSphereCollection::GetSimplePrimitivesArrays(const Volume &clip_vol, const matrix &objrotmatrix, const Cartesian &objorigin, int start, int end) {
  std::vector<Primitive*> a;
  nVerts = 0;
  nRealVerts = 0;
  if(points.size()<1) return a;

  bufferVertices = new float[3*points.size()];
  bufferSizes = new float[points.size()];
  bufferColors = new float[4*points.size()];
  bufferIndices = new GLuint[points.size()];
  
  for(unsigned i=0;i<points.size();i++){
    double size = points[i]->GetSize();
    bufferSizes[i]     = size;
    bufferIndices[i]   = i;
    bufferVertices[3*i]   = points[i]->GetVertices()[0].get_x();
    bufferVertices[3*i+1] = points[i]->GetVertices()[0].get_y();
    bufferVertices[3*i+2] = points[i]->GetVertices()[0].get_z();
    bufferColors[4*i]   = points[i]->GetColour()[0];
    bufferColors[4*i+1] = points[i]->GetColour()[1];
    bufferColors[4*i+2] = points[i]->GetColour()[2];
    bufferColors[4*i+3] = points[i]->GetAlpha();
  }

  nRealVerts = 3*points.size();
  nVerts = points.size();

  polygonType = GL_POINTS;
  return a;
}
...
std::vector<Primitive*> SpheroidCollection::GetSimplePrimitivesArrays(const Volume &clip_vol, const matrix &objrotmatrix, const Cartesian &objorigin, int start, int end) {
  ...
  //std::cout << "SpheroidCollection::GetSimplePrimitivesArrays\n"; std::cout.flush();
  ...
  static int spheres_accus[] = {8,12,20,30,40};


  int my_accu = 8;
  if(sphere_accu>=0&&sphere_accu<5) my_accu = spheres_accus[sphere_accu];
  ...
  for(unsigned k=0;k<nSpheres&&k<prims.size();k++){
    ...
    if(p->GetMatrix().get_rows()==4 && p->GetMatrix().get_columns()==4){
      ...
      double dot1 = cartx.DotProduct(cartx,p->GetXAxis());
      double angle = acos(dot1)*180.0/M_PI;
      //std::cout << "dot: " << dot1 << "\n";
      //std::cout << "Angle 1: " << angle << "\n";
      if(fabs(angle)>0.00001){
        Cartesian rotax = cartx.CrossProduct(cartx,p->GetXAxis());
        rotax.normalize();
        //std::cout << "prim x axis: " << p->GetXAxis() << "\n";
        //std::cout << "rotax: " << rotax << "\n";
        q1 = Quat(rotax,1,-angle);
      }
      m = q1.getInvMatrix();
      //std::cout << "xdiff: " << m * cartx - p->GetXAxis() << "\n";

      yprim = m * carty;
      double dot2 = yprim.DotProduct(yprim,p->GetYAxis());
      //std::cout << "dot2: " << dot2 << "\n";
      angle = acos(dot2)*180.0/M_PI;
      //std::cout << "Angle 2: " << angle << "\n";
      ...
      if(fabs(angle)>0.00001){
        ...
        q1.postMult(q2);
        m = q1.getInvMatrix();
        //std::cout << "ydiff: " << yprim.DotProduct(m * carty , p->GetYAxis()) << "\n";
        if(fabs(yprim.DotProduct(m * carty , p->GetYAxis()))<0.8){
          //std::cout << "Try rotations...\n";
          Quat q3 =  Quat(cartx,1,90.0);
          Quat qn = q1;
          qn.postMult(q3);
          matrix m2 = qn.getInvMatrix();
          //std::cout << "ydiff (after 90): " << yprim.DotProduct(m2 * carty , p->GetYAxis()) << "\n";
          //std::cout << "(" << p->GetYAxis() << ")\n";
          //std::cout << "(" << m2 * carty << ")\n";
          if(fabs(yprim.DotProduct(m2 * carty , p->GetYAxis()))>0.8){
            m = m2;
          }
        }
      }
...
std::vector<Primitive*> Ribbon::GetSimplePrimitives(const Volume &clip_vol, const matrix &objrotmatrix, const Cartesian &objorigin, int start, int end) const {
  ...
  if(RenderQuality::GetRenderQuality()&&0)
    my_accu = 180;
  if(style==0)
    get_elliptical_ribbon(a,vertices_tmp,AB,my_accu,textured,multicolour,colour_vector_tmp,colp,alpha,two_colour);
  else if(style==1)
...
std::vector<Primitive*> ArrowHeadRibbon::GetSimplePrimitives(const Volume &clip_vol, const matrix &objrotmatrix, const Cartesian &objorigin, int start, int end) const {
  ...
  if(RenderQuality::GetRenderQuality()&&0)
    my_accu = 180;
  if(style==0)
    get_elliptical_ribbon(a,vertices_tmp,AB,my_accu,textured,multicolour,colour_vector_tmp,colp,alpha,false);
  else if(style==1)
...
int Primitive::GetNumberOfBufferVertices() const {
  return nRealVerts;
}

int Primitive::GetNumberOfBufferIndices() const {
  return nVerts;
}
