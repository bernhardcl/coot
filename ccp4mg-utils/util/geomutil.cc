...
double  VectorDoubleMean(const std::vector<double> &vals){
...
double  VectorDoubleStandardDeviation(const std::vector<double> &vals){
  if(vals.size()==0) return 0;
  double mean = VectorDoubleMean(vals);
  double sum = 0.0;
  for(unsigned int i=0;i<vals.size();i++){
    sum += (mean-vals[i])*(mean-vals[i]);
  }
