...
extern int ExcludeOverlappedAtoms ( mmdb::Manager* molHnd, const int selHnd, const mmdb::realtype cutoff, int theModel ) ;
...
Cartesian AtToCart(mmdb::Atom* at){
...
std::vector<Cartesian> GetBasePairEnds(mmdb::Residue* res1, mmdb::Residue* res2);
...
int GetCAFromSelection(CMMANManager *molH, int atom_selHnd_in){
  ...
  molH->Select(CALocAselHnd,mmdb::STYPE_ATOM,0,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","CA","C","A",mmdb::SKEY_NEW);
  molH->Select(CALocAselHnd,mmdb::STYPE_ATOM,atom_selHnd_in,mmdb::SKEY_AND);
  int CALocBselHnd = molH->NewSelection();
  molH->Select(CALocBselHnd,mmdb::STYPE_ATOM,0,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","CA","C","B",mmdb::SKEY_NEW);
  molH->Select(CALocBselHnd,mmdb::STYPE_ATOM,atom_selHnd_in,mmdb::SKEY_AND);
  int CALocCselHnd = molH->NewSelection();
  molH->Select(CALocCselHnd,mmdb::STYPE_ATOM,0,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","CA","C","C",mmdb::SKEY_NEW);
  molH->Select(CALocCselHnd,mmdb::STYPE_ATOM,atom_selHnd_in,mmdb::SKEY_AND);
  
  mmdb::Atom** CALocAAtomTable;
  int nCALocAAtoms;
  mmdb::Atom** CALocBAtomTable;
  int nCALocBAtoms;
  mmdb::Atom** CALocCAtomTable;
  int nCALocCAtoms;
  ...
  molH->Select(CAselHnd,mmdb::STYPE_ATOM,atom_selHnd_in,mmdb::SKEY_AND);
  ...
  for(int ia=0;ia<nCALocAAtoms;ia++){
	  for(int ib=0;ib<nCALocBAtoms;ib++){
		  if(CALocAAtomTable[ia]->isInSelection(CAselHnd)&&CALocBAtomTable[ib]->isInSelection(CAselHnd)&&CALocAAtomTable[ia]->residue==CALocBAtomTable[ib]->residue){
			  // Exclude one of them!
			  molH->SelectAtom(CAselHnd,CALocBAtomTable[ib],mmdb::SKEY_XOR,false);
		  }
	  }
  }
  for(int ia=0;ia<nCALocAAtoms;ia++){
	  for(int ib=0;ib<nCALocCAtoms;ib++){
		  if(CALocAAtomTable[ia]->isInSelection(CAselHnd)&&CALocCAtomTable[ib]->isInSelection(CAselHnd)&&CALocAAtomTable[ia]->residue==CALocCAtomTable[ib]->residue){
			  // Exclude one of them!
			  molH->SelectAtom(CAselHnd,CALocCAtomTable[ib],mmdb::SKEY_XOR,false);
		  }
	  }
  }
  ...
  //molH->SelectProperty(NALocAselHnd,SELPROP_Nucleotide,mmdb::STYPE_RESIDUE,mmdb::SKEY_NEW);
  //molH->Select(NALocAselHnd,mmdb::STYPE_ATOM,0,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","*","*","A",mmdb::SKEY_AND);
  molH->Select(NALocAselHnd,mmdb::STYPE_ATOM,0,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","C5*,C5\',C3*,C3\'","C","A",mmdb::SKEY_NEW);
  //molH->Select(NALocAselHnd,mmdb::STYPE_ATOM,0,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","C5\'","C","A",mmdb::SKEY_OR);
  //molH->Select(NALocAselHnd,mmdb::STYPE_ATOM,0,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","C3*","C","A",mmdb::SKEY_OR);
  //molH->Select(NALocAselHnd,mmdb::STYPE_ATOM,0,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","C3\'","C","A",mmdb::SKEY_OR);
  molH->Select(NALocAselHnd,mmdb::STYPE_ATOM,atom_selHnd_in,mmdb::SKEY_AND);
  int NALocBselHnd = molH->NewSelection();
  //molH->SelectProperty(NALocBselHnd,SELPROP_Nucleotide,mmdb::STYPE_RESIDUE,mmdb::SKEY_NEW);
  //molH->Select(NALocBselHnd,mmdb::STYPE_ATOM,0,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","*","*","B",mmdb::SKEY_AND);
  molH->Select(NALocBselHnd,mmdb::STYPE_ATOM,0,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","C5*,C5\',C3*,C3\'","C","B",mmdb::SKEY_NEW);
  //molH->Select(NALocBselHnd,mmdb::STYPE_ATOM,0,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","C5\'","C","B",mmdb::SKEY_OR);
  //molH->Select(NALocBselHnd,mmdb::STYPE_ATOM,0,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","C3*","C","B",mmdb::SKEY_OR);
  //molH->Select(NALocBselHnd,mmdb::STYPE_ATOM,0,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","C3\'","C","B",mmdb::SKEY_OR);
  molH->Select(NALocBselHnd,mmdb::STYPE_ATOM,atom_selHnd_in,mmdb::SKEY_AND);
  int NALocCselHnd = molH->NewSelection();
  //molH->SelectProperty(NALocCselHnd,SELPROP_Nucleotide,mmdb::STYPE_RESIDUE,mmdb::SKEY_NEW);
  //molH->Select(NALocCselHnd,mmdb::STYPE_ATOM,0,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","*","*","C",mmdb::SKEY_AND);
  molH->Select(NALocCselHnd,mmdb::STYPE_ATOM,0,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","C5*,C5\',C3*,C3\'","C","C",mmdb::SKEY_NEW);
  //molH->Select(NALocCselHnd,mmdb::STYPE_ATOM,0,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","C5\'","C","C",mmdb::SKEY_OR);
  //molH->Select(NALocCselHnd,mmdb::STYPE_ATOM,0,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","C3*","C","C",mmdb::SKEY_OR);
  //molH->Select(NALocCselHnd,mmdb::STYPE_ATOM,0,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","C3\'","C","C",mmdb::SKEY_OR);
  molH->Select(NALocCselHnd,mmdb::STYPE_ATOM,atom_selHnd_in,mmdb::SKEY_AND);

  mmdb::Atom** NALocAAtomTable;
  int nNALocAAtoms;
  mmdb::Atom** NALocBAtomTable;
  int nNALocBAtoms;
  mmdb::Atom** NALocCAtomTable;
  int nNALocCAtoms;
  ...
  //molH->SelectProperty(NAselHnd,SELPROP_Nucleotide,mmdb::STYPE_RESIDUE,mmdb::SKEY_NEW);
  //molH->Select(NAselHnd,mmdb::STYPE_ATOM,0,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","*","*","*",mmdb::SKEY_AND);
  molH->Select(NAselHnd,mmdb::STYPE_ATOM,0,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","C5*,C5\',C3*,C3\'","C","*",mmdb::SKEY_NEW);
  //molH->Select(NAselHnd,mmdb::STYPE_ATOM,0,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","C5\'","C","*",mmdb::SKEY_OR);
  //molH->Select(NAselHnd,mmdb::STYPE_ATOM,0,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","C3*","C","*",mmdb::SKEY_OR);
  //molH->Select(NAselHnd,mmdb::STYPE_ATOM,0,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","C3\'","C","*",mmdb::SKEY_OR);
  molH->Select(NAselHnd,mmdb::STYPE_ATOM,atom_selHnd_in,mmdb::SKEY_AND);
  ...
  for(int ia=0;ia<nNALocAAtoms;ia++){
	  for(int ib=0;ib<nNALocBAtoms;ib++){
		  if(NALocAAtomTable[ia]->isInSelection(NAselHnd)&&NALocBAtomTable[ib]->isInSelection(NAselHnd)&&NALocAAtomTable[ia]->residue==NALocBAtomTable[ib]->residue){
			  // Exclude one of them!
			  molH->SelectAtom(NAselHnd,NALocBAtomTable[ib],mmdb::SKEY_XOR,false);
		  }
	  }
  }
  for(int ia=0;ia<nNALocAAtoms;ia++){
	  for(int ib=0;ib<nNALocCAtoms;ib++){
		  if(NALocAAtomTable[ia]->isInSelection(NAselHnd)&&NALocCAtomTable[ib]->isInSelection(NAselHnd)&&NALocAAtomTable[ia]->residue==NALocCAtomTable[ib]->residue){
			  // Exclude one of them!
			  molH->SelectAtom(NAselHnd,NALocCAtomTable[ib],mmdb::SKEY_XOR,false);
		  }
	  }
  }
  ...
  molH->Select(CAselHnd,mmdb::STYPE_ATOM,NAselHnd,mmdb::SKEY_OR);
...
SplineInfo GetSplineInfo (mmdb::Manager *molH, int atom_selHnd_in ,const AtomColourVector &atom_colour_vector,int spline_accu, int udd_chain, int udd_CA, int flatten_beta_sheet, int flatten_loop,int smooth_helix, double trace_cutoff, double loop_frac, int customWidthUDD){
  ...
  mmdb::Atom** atomTable=NULL;
  std::vector<std::vector<mmdb::Atom*> > cavertices;
  std::vector<std::vector<mmdb::Atom*> > nac5vertices;

  Cartesian pos;

  int atom_selHnd = molH->NewSelection();
  if(nAtoms>0){
  molH->Select(atom_selHnd,mmdb::STYPE_ATOM,modelNum,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","*","*","*",mmdb::SKEY_NEW);
  molH->Select(atom_selHnd,mmdb::STYPE_ATOM,atom_selHnd_in,mmdb::SKEY_AND);
  }
  ...
  int CALocAselHnd = molH->NewSelection();
  molH->Select(CALocAselHnd,mmdb::STYPE_ATOM,modelNum,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","CA","C","A",mmdb::SKEY_NEW);
  //molH->Select(CALocAselHnd,mmdb::STYPE_ATOM,atom_selHnd_in,mmdb::SKEY_AND);
#ifdef _DO_TIMINGS_
  tt = clock();
...
#endif
  int CALocBselHnd = molH->NewSelection();
  molH->Select(CALocBselHnd,mmdb::STYPE_ATOM,modelNum,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","CA","C","B",mmdb::SKEY_NEW);
  //molH->Select(CALocBselHnd,mmdb::STYPE_ATOM,atom_selHnd_in,mmdb::SKEY_AND);
#ifdef _DO_TIMINGS_
  tt = clock();
...
#endif
  int CALocCselHnd = molH->NewSelection();
  molH->Select(CALocCselHnd,mmdb::STYPE_ATOM,modelNum,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","CA","C","C",mmdb::SKEY_NEW);
  //molH->Select(CALocCselHnd,mmdb::STYPE_ATOM,atom_selHnd_in,mmdb::SKEY_AND);
#ifdef _DO_TIMINGS_
  tt = clock();
  ...
  molH->Select(CAselHnd_all,mmdb::STYPE_ATOM,modelNum,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","CA","C","*",mmdb::SKEY_NEW);
#ifdef _DO_TIMINGS_
  tt = clock();
  ...
  for(int ia=0;ia<nCALocAAtoms;++ia){
	  for(int ib=0;ib<nCALocBAtoms;++ib){
		  if(CALocAAtomTable[ia]->isInSelection(CAselHnd_all)&&CALocBAtomTable[ib]->isInSelection(CAselHnd_all)&&CALocAAtomTable[ia]->residue==CALocBAtomTable[ib]->residue){
			  // Exclude one of them!
			  molH->SelectAtom(CAselHnd_all,CALocBAtomTable[ib],mmdb::SKEY_XOR,false);
		  }
	  }
  }
#ifdef _DO_TIMINGS_
  ...
  for(int ia=0;ia<nCALocAAtoms;++ia){
	  for(int ib=0;ib<nCALocCAtoms;++ib){
		  if(CALocAAtomTable[ia]->isInSelection(CAselHnd_all)&&CALocCAtomTable[ib]->isInSelection(CAselHnd_all)&&CALocAAtomTable[ia]->residue==CALocCAtomTable[ib]->residue){
			  // Exclude one of them!
			  molH->SelectAtom(CAselHnd_all,CALocCAtomTable[ib],mmdb::SKEY_XOR,false);
		  }
	  }
  }
#ifdef _DO_TIMINGS_
  ...
  molH->Select(CAselHnd,mmdb::STYPE_ATOM,CAselHnd_all,mmdb::SKEY_NEW);
  molH->Select(CAselHnd,mmdb::STYPE_ATOM,atom_selHnd_in,mmdb::SKEY_AND);
#ifdef _DO_TIMINGS_
  tt = clock();
  ...
  int CA_nAtoms_all;
  mmdb::Atom** CA_atomTable_check_all=NULL;
  int CA_nAtoms_thisSel;
  mmdb::Atom** CA_atomTable_check_thisSel=NULL;
  ...
  cavertices.push_back(std::vector<mmdb::Atom*>(0));
  nac5vertices.push_back(std::vector<mmdb::Atom*>(0));
  ...
  int udd_duplchids = molH->GetUDDHandle (mmdb::UDR_HIERARCHY,"duplicate_chain_ids" );
  if (udd_duplchids>0){
    int duplIDSFromUDD;
  ...
  std::vector<std::vector<std::pair<mmdb::Atom*,int> > > cavertices_dupl;
        ...
        if(cavertices.back().size()>0){
          ...
          if(true) {
            ...
            if(old_idupl>=0){
	          ...
	          (pCA->z - pCAprev->z)*(pCA->z - pCAprev->z);
                ...
                if(dist2 <= trace_cutoff_sq && pCA->GetChainID() == pCAprev->GetChainID()){
                  cavertices_dupl[old_idupl].push_back(std::pair<mmdb::Atom*,int>(pCA,j));
                  doneDupl = true;
                }
            }
	          ...
	          (pCA->z - pCAprev->z)*(pCA->z - pCAprev->z);
                ...
                if(dist2 <= trace_cutoff_sq && pCA->GetChainID() == pCAprev->GetChainID()){
                  if(cavertices_dupl[idupl].size()==1&&old_idupl>=0){
                     ...
                     mmdb::Atom* pCAoldChain = cavertices_dupl[old_idupl].back().first;
	               ...
	               (pCAoldChain->z - pCAprev->z)*(pCAoldChain->z - pCAprev->z);
                      ...
                      if(dist2 <= trace_cutoff_sq && pCAoldChain->GetChainID() == pCAprev->GetChainID()){
                        ...
                        std::pair<mmdb::Atom*,int> oldsing = cavertices_dupl[idupl].back();
                  ...
                  if(!doneDupl){
                    cavertices_dupl[idupl].push_back(std::pair<mmdb::Atom*,int>(pCA,j));
                    doneDupl = true;
                  }
                }
              }
            ...
            if(!doneDupl){
              ...
              cavertices_dupl.push_back(std::vector<std::pair<mmdb::Atom*,int> >(0));
              cavertices_dupl.back().push_back(std::pair<mmdb::Atom*,int>(pCA,j));
  ...
  for(unsigned idupl=0;idupl<cavertices_dupl.size();idupl++){
    ...
    nChain++;
    if(idupl>0) cavertices.push_back(std::vector<mmdb::Atom*>(0));
    splineinfo.colours.push_back(std::vector<Cartesian>(0));
    ...
    //if(iatdupl==0) std::cout << cavertices_dupl[idupl][0].first->GetAtomID(AtomID) << "\n";
	  ...
	  if(iatdupl==0){
	    ...
	    delete [] atcol;
            ...
            if(((splineinfo.secstr_indices.back().back().back()==(int)mmdb::SSE_Strand||splineinfo.secstr_indices.back().back().back()==(int)mmdb::SSE_Bulge)
              &&((int)pRes->SSE!=(int)mmdb::SSE_Strand)&&(int)pRes->SSE!=(int)mmdb::SSE_Bulge)
              ||(splineinfo.secstr_indices.back().back().back()==(int)mmdb::SSE_Helix
              &&((int)pRes->SSE!=(int)mmdb::SSE_Helix))
               ){
	      if((int)pRes->SSE==(int)mmdb::SSE_Helix||(int)pRes->SSE==(int)mmdb::SSE_Strand||(int)pRes->SSE==(int)mmdb::SSE_Bulge){
        ...
        if(iatdupl==0|| ( int(pRes->SSE)!=splineinfo.secstr_indices.back().back()[1] &&
          !( ( int(pRes->SSE)== mmdb::SSE_Strand || int(pRes->SSE)== mmdb::SSE_Bulge) &&
              (splineinfo.secstr_indices.back().back()[1] == mmdb::SSE_Strand 
                 || splineinfo.secstr_indices.back().back()[1] == mmdb::SSE_Bulge )) )  ) {
          splineinfo.secstr_indices.back().push_back(std::vector<int>(0));
          //std::cout << nPept << "\n";
          splineinfo.secstr_indices.back().back().push_back(nPept);
          if (pRes->SSE == mmdb::SSE_Bulge) {
	    splineinfo.secstr_indices.back().back().push_back(int(mmdb::SSE_Strand));
	  } else {
  ...
  if(!allAtoms&&!duplIDS){
  for(unsigned ii=0; ii<cavertices.size(); ++ii){
   std::list<mmdb::Atom*> prev_atom_p;
   if(cavertices[ii].size()>1){
    
    int CA_this_chain_selHnd = molH->NewSelection();
    molH->Select(CA_this_chain_selHnd,mmdb::STYPE_ATOM,modelNum,cavertices[ii][0]->GetChainID(),
		 mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","CA","C","*",mmdb::SKEY_NEW);
    molH->Select(CA_this_chain_selHnd,mmdb::STYPE_ATOM,CAselHnd_all,mmdb::SKEY_AND);
    ...
    mmdb::Atom* first_at = cavertices[ii][0];
    ...
    while(jat<4&&in_this_chain&&curr_resno>0&&curr_at->isInSelection(CA_this_chain_selHnd)){
      if(CA_atomTable[curr_resno-1]&&CA_atomTable[curr_resno]){
         mmdb::Atom* pCA1 = CA_atomTable[curr_resno];
         mmdb::Atom* pCA2 = CA_atomTable[curr_resno-1];
   ...
   if(prev_atoms[ii]>0){
    std::vector<mmdb::Atom*> new_ca0;
    std::list<mmdb::Atom*>::iterator patom_iter = prev_atom_p.begin();
  ...
  for(unsigned ii=0; ii<cavertices.size(); ++ii){
   if(cavertices[ii].size()>1){
    ...
    molH->Select(CA_this_chain_selHnd,mmdb::STYPE_ATOM,modelNum,cavertices[ii].back()->GetChainID(),
		 mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","CA","C","*",mmdb::SKEY_NEW);
    molH->Select(CA_this_chain_selHnd,mmdb::STYPE_ATOM,CAselHnd_all,mmdb::SKEY_AND);

    molH->GetSelIndex ( CA_this_chain_selHnd, CA_atomTable, CA_nAtoms );
    //std::cout << "Sequence number of last atom " << cavertices[ii].back()->GetResidueNo() << "\n";
    mmdb::Atom* first_at = cavertices[ii].back();
    int resno = first_at->GetResidueNo();
  ...
  mmdb::Atom* c5prev = 0;
  for(int j=0;j<nAtoms;++j){
    mmdb::Residue* res = atomTable[j]->GetResidue();
    if(res){
      if ((res->GetAtom("C3'","*","*") || res->GetAtom("C3*","*","*")) &&
             (res->GetAtom("C5'","*","*") || res->GetAtom("C5*","*","*")) &&
	      res->GetAtom("N1","*","*") && res->GetAtom("C2","*","*") && res->GetAtom("C4","*","*")) {
        mmdb::Atom* c5 = res->GetAtom("C5\'");
        ...
        if(nac5vertices.back().size()>0&&c5){
          mmdb::Atom* nac5prev = nac5vertices.back().back();
          ...
          if(distnca> 9.5 ||(c5prev&&c5->GetChainID()!=c5prev->GetChainID())){
            mmdb::Residue* resp = c5prev->GetResidue();
            mmdb::Atom* c3p = resp->GetAtom("C3\'");
  ...
  if(c5prev&&nac5vertices.size()>0&&nac5vertices.back().size()>0){
    mmdb::Atom* c3 = c5prev->GetResidue()->GetAtom("C3\'");
  ...
  std::vector<std::vector<mmdb::Atom*> >::iterator mna=nac5vertices.begin();  
  while(mna!=nac5vertices.end()){
    std::vector<mmdb::Atom*>::iterator l=(*mna).begin();
	...
	if(n1&&c2&&c4){
	   ...
	   Cartesian c4_cart = AtToCart(c4);
           ...
           if(strncmp((*l)->GetResidue()->name,"PSU",3)==0){
             mmdb::Atom* c1p = (*l)->GetResidue()->GetAtom("C1\'");
             ...
             if(!c1p||!c1p->isInSelection(atom_selHnd_in)) c1p = (*l)->GetResidue()->GetAtom("C1*",NULL,"B");
             mmdb::Atom* c2p = (*l)->GetResidue()->GetAtom("C2\'");
  ...
  std::vector<std::vector<mmdb::Atom*> >::iterator m=cavertices.begin();  
  int ichain = 0;
  //std::cout << "GetSplineInfo 12" << "\n";
  while(m!=cavertices.end()){
    //std::cout << "Starting to build a spline of size " << (*m).size() << "\n"; std::cout.flush();
    std::vector<mmdb::Atom*>::iterator l=(*m).begin();
    ...
    if(m->size()>2){
      ...
      while(l!=(*m).end()){
        ...
        bool curr_beta = (int((*l)->GetResidue()->SSE)==mmdb::SSE_Strand || int((*l)->GetResidue()->SSE)==mmdb::SSE_Bulge);
        if(l+1<(*m).end()) curr_beta = (int((*(l+1))->GetResidue()->SSE)==mmdb::SSE_Strand || int((*(l+1))->GetResidue()->SSE)==mmdb::SSE_Bulge)&&(int((*l)->GetResidue()->SSE)==mmdb::SSE_Strand || int((*l)->GetResidue()->SSE)==mmdb::SSE_Bulge);
        ...
        if(smooth_helix){
          bool curr_helix = (int((*l)->GetResidue()->SSE)==mmdb::SSE_Helix );
        ...
        if(flatten_loop){
          bool curr_loop = (int((*l)->GetResidue()->SSE)==mmdb::SSE_None || (int((*l)->GetResidue()->SSE)!=mmdb::SSE_Strand && int((*l)->GetResidue()->SSE)!=mmdb::SSE_Bulge && int((*l)->GetResidue()->SSE)!=mmdb::SSE_Helix) );
        ...
        if(l<(*m).begin()+2){
          ...
          if(Cartesian::DotProduct(n1,n1_ctrlpoints.back())<0.0){
            if((l!=(*m).begin()&&(*l)->GetResidue()->SSE==mmdb::SSE_Helix&&(*(l-1))->GetResidue()->SSE==mmdb::SSE_Helix)){
  ...
  for(unsigned ichain=0;ichain<secstr_indices.size();++ichain){
	...
	secstr_indices_new.back().push_back(*sec_ind_iter);
        if((*sec_ind_iter)[1]==mmdb::SSE_Strand||(*sec_ind_iter)[1]==mmdb::SSE_Bulge||(*sec_ind_iter)[1]==mmdb::SSE_Helix){
          if((*(sec_ind_iter+1))[1]==mmdb::SSE_Strand||(*(sec_ind_iter+1))[1]==mmdb::SSE_Bulge||(*(sec_ind_iter+1))[1]==mmdb::SSE_Helix){
...
SplineInfo GetRibbonOrWormSplineInfo(mmdb::Manager* molhnd, int side_to_ribbon_worm, bool isribbon){
  mmdb::Atom** atomTable=0;
  int nAtoms;
  molhnd->GetAtomTable ( atomTable, nAtoms );  
  int spline_accu = 4;
  int udd_chain,udd_CA;
  udd_chain = molhnd->GetUDDHandle ( mmdb::UDR_ATOM,"spline_tmp_atom_int" );
  udd_CA = molhnd->GetUDDHandle ( mmdb::UDR_ATOM,"spline_tmp_atom_int1" );
  if (udd_chain<=0) udd_chain = molhnd->RegisterUDInteger(mmdb::UDR_ATOM,"spline_tmp_atom_int" );
  if (udd_CA<=0) udd_CA = molhnd->RegisterUDInteger(mmdb::UDR_ATOM,"spline_tmp_atom_int1" );
...
std::vector<std::vector<Cartesian> > GetExternalCartesians_internal(mmdb::Manager* molhnd, const std::vector<std::vector<int> > &ext_conn_lists, const SplineInfo &splineinfo_ribbon, const SplineInfo &splineinfo_worm, int side_to_ribbon, int side_to_worm, double trace_cutoff ){
...
std::vector<std::vector<Cartesian> > GetExternalCartesiansWithSplineInfo(mmdb::Manager* molhnd, const std::vector<std::vector<int> > &ext_conn_lists, const SplineInfo &splineinfo_ribbon, const SplineInfo &splineinfo_worm, int side_to_ribbon, int side_to_worm, double trace_cutoff){
...
std::vector<std::vector<Cartesian> > GetExternalCartesians(mmdb::Manager* molhnd, const std::vector<std::vector<int> > &ext_conn_lists, int side_to_ribbon, int side_to_worm, double trace_cutoff ){
  SplineInfo splineinfo_ribbon;
  SplineInfo splineinfo_worm;
  return GetExternalCartesians_internal(molhnd, ext_conn_lists, splineinfo_ribbon, splineinfo_worm, side_to_ribbon, side_to_worm, trace_cutoff);
}
