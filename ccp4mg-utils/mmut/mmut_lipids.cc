...
int  MMUTLipidAnalyse(CMMUTManager *molHnd, int selHnd_in, int minimum_chain_length){
  ...
  int nAtoms_all;
  mmdb::Atom** atomTable_all=0;
  
  int selHnd = molHnd->NewSelection();
  molHnd->Select  (selHnd,mmdb::STYPE_ATOM,selHnd_in,mmdb::SKEY_NEW);

  int selHnd_all = molHnd->NewSelection();
  molHnd->Select  (selHnd_all,mmdb::STYPE_ATOM,selHnd,mmdb::SKEY_NEW);
  ...
  molHnd->Select  (selHnd,mmdb::STYPE_ATOM,1,chainID,mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","*","C","*",mmdb::SKEY_AND);
  molHnd->Select  (selHnd,mmdb::STYPE_ATOM,1,chainID,mmdb::ANY_RES,"*",mmdb::ANY_RES,"*",excluded_residues,"*","*","*",mmdb::SKEY_XOR);
  atomTable=0;
  molHnd->GetSelIndex ( selHnd, atomTable, nAtoms );
...
std::vector<MMUTLipid> MMUTLipidCalculate(CMMUTManager *molHnd, int selHnd_in, int minimum_chain_length){
  clock_t t1 = clock();
  ...
  int selHnd_all = molHnd->NewSelection();
  molHnd->Select  (selHnd_all,mmdb::STYPE_ATOM,1,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","*","*","*",mmdb::SKEY_NEW);
  //molHnd->Select  (selHnd_all,mmdb::STYPE_ATOM,selHnd,mmdb::SKEY_NEW);
  //molHnd->GetSelIndex ( selHnd_all, atomTable_all, nAtoms_all );
  ...
  molHnd->Select  (selHnd,mmdb::STYPE_ATOM,1,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*","*","*","C","*",mmdb::SKEY_AND);
  molHnd->Select  (selHnd,mmdb::STYPE_ATOM,1,"*",mmdb::ANY_RES,"*",mmdb::ANY_RES,"*",excluded_residues,"*","*","*",mmdb::SKEY_CLR);
  atomTable=0;
  molHnd->GetSelIndex ( selHnd, atomTable, nAtoms );
  ...
  std::vector<std::vector<std::vector<int> > > conn_order_lists_all = conn_all.GetConnectivityLists();
  std::vector<std::vector<int> > conn_lists_all = conn_order_lists_all[0];
  Tree tree_all;
...
*/
  ...
  for(unsigned ii=0;ii<branches.size();ii++){
    ...
    mmdb::Atom* branch_start_atom = atomTable[branch_start];
    mmdb::Atom* branch_end_atom = atomTable[branch_end];
  ...
  if(head_groups.size()>0){
    for(unsigned jj=0;jj<head_groups.size();jj++){
      ...
      for(unsigned ii=0;ii<head_groups[jj].size();ii++){
        ...
        mmdb::Atom* at = atomTable_all[head_groups[jj][ii]];
  ...
  for(unsigned jj=0;jj<branches.size();jj++){
    ...
    for(unsigned ii=0;ii<branches[jj].size();ii++){
      mmdb::Atom* at = atomTable[branches[jj][ii]];
      //ivec[ii] = at->serNum;
    ...
    for(unsigned ii=0;ii<extra_branch_nodes[jj].size();ii++){
      ...
      mmdb::Atom* at = atomTable_all[extra_branch_nodes[jj][ii]];
