...
//----------------------------------------------------------------------------
  Atom** selAtoms;
  Residue **selRes=NULL,**resTable=NULL;
...
//----------------------------------------------------------------------
  ...
  const char *restyps[] = { "*","PHE", "TRP", "TYR", "PRO", "VAL",
		     "ALA", "ILE", "LEU", "SER", "THR",
		     "ASN", "GLN", "ARG", "LYS", "ASP",
		     "GLU", "CYS", "MET", "GLY", "HIS",
		     "A",   "T"  , "G"  , "C"  , "U",
		     "DA",   "DT"  , "DG"  , "DC"  ,
		     "ADE", "THY", "GUA", "CYT", "URA",
                     "BMA", "MAN", "NAG","GLC","BGC","GCS",
                     "GAL", "NGA", "MGC",
                     "NG1", "NG6", "NGA",
                     "6MN","GLP","GP4",
                     "BEM",
                     "KDN",
                     "XLS","CXY","RBY","TDX","XYL","XYS",
                     "FCA","FCB","FUC",
                     "GTR","ADA","DGU",
                     "SI3","NCC",
                     "NGF","NGE","NGC",
                     "GC4","GCD","GCU","GCV","GCW","IDS","REL",
                     "IDR"
                     } ;
  const char *rescols[] = {  "grey","magenta", "magenta", "magenta", "coral", "coral",
  ...
  RC = ResType->SetSchemeString ( sizeof(rescols)/sizeof(char*), restyps, rescols );
...
//-----------------------------------------------------------------------
  ...
  if ( value < ranges[1] ) { 
    ...
    for ( n = 1; n<nTypes-2;n++) {
      if ( value < ranges[n+1] || fabs(value - ranges[n+1])<1e-5 ) {
        fcolour = float(n-1) + 
