...
class CColourScheme {
 ...
 public:
  ...
  int SetSchemeFloat ( int n, mmdb::realtype typ[] , const char *cols[] );
...
public :
 ...
 // Destructor
  ...
  int SetMode ( int force, int mod=-1 , int non_C_mod=-2, int icol = -1, int ncols=0, int *icols = NULL );
  ...
  mmdb::ivector rules_colours;
  mmdb::ivector rules_selHnds;
  AtomColourVector atom_colour_vector;
  std::vector<int> colour_wheel_direction;

  int OneColour ();
  int ByAtomType ( );
  int ByAtomType (int colour_carbon, int *colour, int nat, mmdb::Atom** selAtoms);
  int ByResidueType ();
  int ByChain();
