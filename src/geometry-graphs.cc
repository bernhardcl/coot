...
coot::on_geometry_graph_block_clicked(GtkCanvasItem *item, GdkEvent *event, gpointer data) { 
   ...
   if (event->motion.state & GDK_BUTTON1_MASK) {
	 ...
	 coot::geometry_graphs *graphs = 
       (coot::geometry_graphs *) (g_object_get_data(G_OBJECT(canvas), "user_data"));
...
coot::geometry_graphs::render_b_factor_blocks(int imol,
   ...
   render_b_factor_blocks_with_scale(imol, chain_number, chain_id,
                                     offset, biv, 1.);
...
void
coot::geometry_graphs::render_b_factor_blocks_with_scale(int imol,
					      int chain_number,
					      const std::string  &chain_id,
					      int offset, 
                     const std::vector<coot::b_factor_block_info_t> &biv,
...
// 	     << std::endl;
   ...
   g_object_set_data_full(G_OBJECT(dialog), "geometry_graph_canvas", canvas,
             (GDestroyNotify) g_object_unref);
   g_object_set_data(G_OBJECT(canvas), "user_data", (char *) this);
...
//    std::cout << "plotting " << blocks[chain_number].size() << std::endl;
   ...
   g_signal_connect(G_OBJECT(item), "event",
            (GCallback) coot::on_geometry_graph_block_clicked, // dangerous?
